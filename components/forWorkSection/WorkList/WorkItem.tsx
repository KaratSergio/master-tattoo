...
import { EllipseBlue } from '@/components/icons/ellipse/workSection/EllipseBlue';
...
const WorkItem: FC<WorkItemProps> = ({ title, text, number }) => {
    ...
    const circlePositions: Record<string, string[]> = {
        '1': [
            'bottom-[-10px] left-0 z-[-1]',
            'bottom-5 left-[-4px] size-1',
            'top-[-12px] left-1 size-4',
            'bottom-4 left-16 size-6',
        ],

        '2': ['top-8 left-[-12px] size-5', 'top-6 left-14 size-3', 'bottom-0 left-10 z-[-1]'],

        '3': [
            'bottom-[-12px] left-[-6px] z-[-1]',
        ...
        '5': ['top-2 right-2 size-3', 'top-6 right-6 size-4', 'top-10 left-10 z-[-1]'],

        '6': [
            'top-[-5px] left-[5px] z-[-1]',
            'bottom-[-20px] left-[15px] z-[-1]',
        ...
        <li className="flex items-center gap-6">
            <div className="relative work-item-bg size-20 content-center pt-2">
                {positions.map((position, index) => (
                    <EllipseBlue key={index} className={`absolute ${position}`} />
                ))}
